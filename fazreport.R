...
runs <- c(219, 235)
ci.run <- c(235) # which run has confidence intervals 
		...
		if(run %in% ci.run) {
			for(year in years[which(years>=2020)[1]:lyears]) {
				ci.file.name <- file.path(ci.dir, 
									paste('run', run, '_quantiles_', geography, sep=''), 
									paste(geography, '_',
...
for(faz in sort(fazids[[indicators[1]]][[runs[1]]])) {
	...
	for (what in indicators) {
		...
		for(irun in 1:length(runs)) {
			idx <- which(fazids[[what]][[runs[irun]]]==faz)
			if (length(idx) <=0) {faz_not_found <- TRUE; break}
			coln <- colnames(sim[[what]][[runs[irun]]])
			run.cols <- substr(coln, nchar(coln)-3, nchar(coln))
			col.idx <- which(run.cols %in% as.character(years))
			this.data <- data.frame(run=rep(runs[irun], lyears), Time=years, 
							amount=as.numeric(c(rep(NA, lyears-length(col.idx)), 
										sim[[what]][[runs[irun]]][idx,col.idx])))
			if(length(CIs[[what]][[runs[irun]]])>0) {
				ci.low <- ci.high <- c(rep(NA, sum(years<=2000)), sim[[what]][[runs[irun]]][idx,2])
				for(year in years[3:lyears]) {
					idxci <- which(CIs[[what]][[runs[irun]]][[year]][,'id']==faz)
					ci.low <- c(ci.low, CIs[[what]][[runs[irun]]][[year]][idxci,'lower_80'])
					ci.high <- c(ci.high, CIs[[what]][[runs[irun]]][[year]][idxci,'upper_80'])
				}
			...
			colnames(this.tabdata) <- c(paste('run', runs[irun]), 
										paste('low', runs[irun]), paste('high', runs[irun]))
			run.table.columns <- c(run.table.columns, paste('run', runs[irun]))
			if(!(runs[irun] == ci.run[length(ci.run)])) {
				this.tabdata <- this.tabdata[,1, drop=FALSE]
			} else last.table.columns <- c(paste('low', runs[irun]), paste('high', runs[irun]))
